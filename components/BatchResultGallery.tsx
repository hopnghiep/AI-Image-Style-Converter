...
const BatchResultItem: React.FC<{
    ...
    return (
        <div className="relative group bg-white rounded-xl overflow-hidden border border-gray-200 shadow-sm transition-all hover:shadow-md flex flex-col animate-fade-in">
            <div className="aspect-square w-full flex items-center justify-center p-1.5 bg-[#f9f9f9] relative">
                {result.imageUrl ? (
                    <img src={result.imageUrl} alt={displayName} className="object-contain max-w-full max-h-full rounded-lg transition-all duration-300" />
                ) : result.error ? (
                    <div className="text-center p-1 text-red-500">
                        <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 mx-auto mb-1" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" /></svg>
                        <p className="text-[8px] font-bold uppercase">{language === 'vi' ? 'Lỗi' : 'Failed'}</p>
                    </div>
                ) : (
                    <div className="flex flex-col items-center justify-center gap-1.5">
                        <LoadingSpinner />
                        <p className="text-[8px] text-gray-400 font-bold uppercase animate-pulse">
            ...
            <div className="px-2 py-1.5 bg-white border-t border-gray-50">
                <p className="text-[10px] font-black text-gray-500 truncate uppercase tracking-tighter">{displayName}</p>
            </div>

            {result.imageUrl && (
                 <div className="absolute inset-0 bg-black/70 opacity-0 group-hover:opacity-100 flex items-center justify-center gap-1 transition-all duration-300 p-2">
                    <button 
                        onClick={() => onPreview(result.imageUrl!)} 
                        className="flex-1 h-full bg-white/90 hover:bg-white text-gray-800 rounded-lg font-black text-[9px] uppercase flex flex-col items-center justify-center gap-1 shadow-sm transition-colors"
                    >
                        <span>{language === 'vi' ? 'Xem' : 'View'}</span>
                    </button>
                    <button 
                        onClick={handleDownload} 
                        className="flex-1 h-full bg-[#4A6B5D] hover:bg-[#3e5a4e] text-white rounded-lg font-black text-[9px] uppercase flex flex-col items-center justify-center gap-1 shadow-sm transition-colors"
                    >
...
const BatchResultGallery: React.FC<BatchResultGalleryProps> = ({ results, styles, onClear, onImagePreview, language = 'vi' }) => {
  ...
  return (
    <div className="w-full animate-fade-in p-6 bg-white/40 rounded-3xl border border-gray-200 shadow-inner">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 border-b border-gray-100 pb-4">
            <div>
              <h2 className="text-xl md:text-2xl font-black text-[#4A6B5D] tracking-tighter uppercase">
                  {language === 'vi' ? 'Kết quả chuyển đổi' : 'Conversion Results'}
              </h2>
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-1">
                {results.length} {language === 'vi' ? 'tác phẩm được tạo' : 'artworks generated'}
              </p>
            </div>
